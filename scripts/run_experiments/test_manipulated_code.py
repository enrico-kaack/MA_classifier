...
def run_all_tasks(validation_source, workers):
    ...
    for problem_type in enumerate([ProblemType.RETURN_NONE, ProblemType.CONDITION_COMPARISON_SIMPLE]):
        ...
        t= [            
            ...
            TaskTrainLstm(problem_type=problem_type, oversampling_enabled=False, undersampling_enabled=True, ratio_after_undersampling=0.5, embedding_vecor_length=32, epochs=3, batch_size=256, encode_type=False, dropout_emb_lstm=0.2, dropout_lstm_dense=0.2),
    ...
    for model_task in task_list_ensemble:
        ...
        t = TaskEvalEnsemble(model=model, input_src_path=validation_source, problem_type=problem_type, encode_type=model_task.encode_type, training_parameter={**model_task.__dict__["param_kwargs"], "task_id": model_task.task_id})
        d6tflow.run(t, workers=4)
    ...
    for model_task in task_list_keras:
        ...
        if problem_type == ProblemType.CONDITION_COMPARISON_SIMPLE:
                problem_type = ProblemType.CONDITION_COMPARISON #using the trained model on simple to predict not simple stuff
        t = TaskEvalKeras(model=model, input_src_path=validation_source, problem_type=problem_type, training_parameter={**model_task.__dict__["param_kwargs"], "task_id": model_task.task_id})
