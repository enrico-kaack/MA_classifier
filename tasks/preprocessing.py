...
#    train_dev_split_percentage = luigi.FloatParameter(default=0.0)
    ...
    def requires(self):
        return {
            "train": TaskPrepareXY(input_src_path=self.train_dir, vocab_directory=self.train_dir, problem_type=self.problem_type, window_size=self.window_size, step_size=self.step_size, encode_type=self.encode_type, max_vocab_size=self.max_vocab_size),
